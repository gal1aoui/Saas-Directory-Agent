...
from app.schemas import (
    ...
    SubmissionWithDetails, BulkSubmissionRequest, DashboardStats
)
...
async def get_dashboard_stats(db: Session = Depends(get_db)):
    ...
    pending = db.query(func.count(Submission.id)).filter(Submission.status == SubmissionStatus.PENDING).scalar()
    submitted = db.query(func.count(Submission.id)).filter(Submission.status == SubmissionStatus.SUBMITTED).scalar()
    approved = db.query(func.count(Submission.id)).filter(Submission.status == SubmissionStatus.APPROVED).scalar()
    failed = db.query(func.count(Submission.id)).filter(Submission.status == SubmissionStatus.FAILED).scalar()
    ...
    active_directories = db.query(func.count(Directory.id)).filter(Directory.status == "active").scalar()
