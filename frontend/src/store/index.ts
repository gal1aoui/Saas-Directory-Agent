...
export const queryKeys = {
  ...
  directories: (filters?: DirectoryFilters) => ['directories', filters] as const,
  directory: (id: number) => ['directories', id] as const,
  submissions: (filters?: SubmissionFilters) => ['submissions', filters] as const,
...
export function useUpdateSaasProduct() {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: queryKeys.saasProducts });
      queryClient.invalidateQueries({ queryKey: queryKeys.saasProduct(variables.id) });
    },
  });
}
...
export function useUpdateDirectory() {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['directories'] });
      queryClient.invalidateQueries({ queryKey: queryKeys.directory(variables.id) });
    },
  });
}
